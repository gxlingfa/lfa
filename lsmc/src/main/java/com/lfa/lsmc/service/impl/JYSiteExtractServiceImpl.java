...
@Slf4j
@Service("jySiteExtractService")
public class JYSiteExtractServiceImpl implements SiteExtractService {
    @Value("${jy_source_url}")
    private String sourceUrl;
    @Value("${jy_source_root}")
    private String sourceRoot;

    @Override
    public List<Notice> extract() {
        log.info("抓取揭阳土地资源公告信息。。。。。。");
        List<Notice> notices=new ArrayList<>();
        try{
        Document doc= Jsoup.connect(sourceUrl).timeout(600000).execute().parse();
        Elements lis=doc.select("#categorypagingcontent").select("li");
        Date today=new Date();
        for(Element li:lis){
            Element aE=li.selectFirst("div").selectFirst("a");
            String url=sourceRoot+aE.attr("href");
            String title=aE.text().trim();
            String dateStr=li.selectFirst("span").selectFirst(".ewb-data-date").text();
            ...
            notice.setSource(Notice.Source.JY);
            notice.setUrl(url);
